...
def download_thumbnail(url, output_path, cookies_path=None):
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:            print("Downloading thumbnail...")
            ydl.download([url])
...
def download_video(url, output_path, video_format, audio_format, container, cookies_path=None):
    ...
    if cookies_path:
        ydl_opts['cookiefile'] = cookies_path
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:            print("Downloading video...")
            ydl.download([url])
    except Exception as e:
        print(f"An error occurred while downloading video: {e}")
def validate_url(url):
    youtube_regex = re.compile(r'(https?://)?(www\.)?(youtube|youtu|youtube-nocookie)\.(com|be)/.+$')
    return re.match(youtube_regex, url) is not
None
...
'vcodec' in f and f['vcodec'] != 'none']
        audio_formats = [f for f in formats if
'acodec' in f and f['acodec'] != 'none']
...
def main():
    ...
    for vf in video_formats:
        size = f"{vf['filesize'] // (1024 * 1024)} MB" if 'filesize' in vf else 'Unknown size'        print(f"{vf['format_id']}: {vf['ext']}
- {vf['resolution']} - {vf.get('tbr', 'N/A')}k
- {size}")
    print("\nAvailable Audio Formats:")
    for af in audio_formats:
        size = f"{af['filesize'] // (1024 * 1024)} MB" if 'filesize' in af else 'Unknown size'        print(f"{af['format_id']}: {af['ext']}
- {af['abr']}k - {size}")
    video_format = input("\nEnter Video Format
ID: ")
